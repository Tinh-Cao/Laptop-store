 ...
 * @author msi
 */
@WebServlet(name = "DeleteCateServlet", urlPatterns = {"/editCate"})
public class EditCateServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (cateDAO.getCategoryByColumnName("CategoryID", id_raw).getStatus() == 0) {
                cateDAO.changeStatusCategoryByID(1, id);
                response.sendRedirect("tableCategory");
            } else {
                cateDAO.changeStatusCategoryByID(0, id);
                response.sendRedirect("tableCategory");
            }
